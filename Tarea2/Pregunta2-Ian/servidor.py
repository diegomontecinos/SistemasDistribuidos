...
class ServerChat():
    ...
    def RecibirMSG(self, ch, method, prop, body):
        ...
        if tipo == str(0): #solicita ID
            print('mando: ',cola,' ',emisor)
            self.AddClient(emisor)
            ...
            self.EnviarMensaje(MSG,emisor,receptor,'1')
    ...
    def AddClient(self, cola):
        ...
        print ('diccionario mensajes ', self.directorioUser)
        ...
        MSG ="{emisor;Cliente-0,receptor;[Cliente-X o Server],time;dd-mmm-yyy|hh:mm:ss,mensaje;"+NewID+",COLA;"+cola+",tipo;0}" 
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(host='localhost')
        )
        channel = connection.channel()
        channel.basic_publish(exchange='',routing_key=str(cola),body=str(MSG))
        connection.close()
